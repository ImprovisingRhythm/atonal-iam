...
export class OtpProvider {
  ...
  async generateSecret(userId: ObjectId, captchaToken: string) {
    await captchaProvider.instance.verifyToken(captchaToken, `uid:${userId}`)
  ...
  async getKeyUri(userId: ObjectId, captchaToken: string) {
    await captchaProvider.instance.verifyToken(captchaToken, `uid:${userId}`)
